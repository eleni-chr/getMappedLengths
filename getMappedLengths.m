...
%map_lengths:       An M x 1 matrix of numbers, where M is equal to the
...
for f=1:d %loop through each TXT file.
    ...
    map_lengths=zeros(length(seqs),1); %initialise variable.
    totalreads=length(seqs); %total number of unmapped reads.
    for ii=1:totalreads %loop through each unamapped read.
        map_lengths(ii)=length(seqs{ii}); %calculate length of current sequence and save it in array of lengths.
    end
    ...
    save('length_analysis.mat','map_lengths','-append'); %save the variable.
